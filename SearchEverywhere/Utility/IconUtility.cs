...
public class IconUtility
{
    ...
    public static BitmapImage GetIcon(string path)
    {
        ...
        var rawImg = GetImage(path, true);
        var size = GetApproximateSize(rawImg);
        if (size < 1000) rawImg = GetImage(path, false);
        return ConvertToBitmapImage(rawImg);
    }

    private static Bitmap GetImage(string path, bool isLarge)
    {
        var hIcon = GetJumboIcon(GetIconIndex(path), isLarge);
        using var ico = (Icon) Icon.FromHandle(hIcon).Clone();
        Shell32.DestroyIcon(hIcon); // don't forget to cleanup
        return ico.ToBitmap();
    }

    private static long GetApproximateSize(object o)
    {
    ...
    private static BitmapImage ConvertToBitmapImage(Bitmap img)
    {
    ...
    // 256*256
    private static IntPtr GetJumboIcon(int iImage, bool isLarge)
    {
